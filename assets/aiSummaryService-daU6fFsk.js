import{N as te,Q as ce}from"./index-DarNpAzJ.js";import"./vendor-react-D2fMnCrr.js";import"./vendor-dexie-2jmnBxhj.js";import"./vendor-lucide-BkG-0JyN.js";import"./vendor-jszip-Bm-qbvBf.js";/* empty css                     */import"./vendor-supabase-B9WqnyDd.js";const le=12e4,ue=(e="")=>/timed out|timeout|abort/i.test(String(e)),ie=(e,c=le)=>{const o=String((e==null?void 0:e.message)||e||"").trim();return ue(o)?`请求超时（${Math.round(c/1e3)} 秒）。总结内容较长时可能需要更久，请重试，或缩小总结消息范围后再试。`:/empty response/i.test(o)?"AI 返回了空内容，未能生成有效总结。请重试一次。":/API settings not configured/i.test(o)?"API 尚未配置完整，请先检查 API 地址、密钥和模型设置。":o||"未知错误"},re=(e,c,o,g,u=!1)=>e.map((n,f)=>{const d=c+f+1,i=n.sender==="me"?g:o;let s="";if(n.isVoice&&n.voiceText?s=n.voiceText:n.locationData?s=`[分享了位置: ${n.locationData.name}]`:n.transferData?s=`[转账 ${n.transferData.amount} 元${n.transferData.status==="rejected"?"(已拒收)":""}]`:n.giftData?s=`[送出了${n.giftData.name}]`:n.type==="combined_history"?s="[转发了聊天记录]":n.image?s=n.text?`[发送图片] ${n.text}`:"[发送了图片]":n.sticker?s=`[发送了表情包: ${n.text||"表情"}]`:n.isHtml||(s=n.text||""),s&&!n.isHtml){let r="";if(u){const a=`${n.date||""} ${n.time||""}`.trim();a&&(r=`[${a}] `)}return`${r}#${d} ${i}: ${s}`}return""}).filter(n=>n).join(`
